// Mirror of agent-worker/src/phases/v6SystemPrompt.ts for PoC use.
// Keep in sync manually — when prod prompt changes, copy the content here.
// Source of truth is still the .ts file; this is the PoC's harness-only copy.
...
export function buildV6UserMessage({ canvasWidth, canvasHeight, userPrompt, trendContext }) {
  const base = `Canvas: ${canvasWidth}px × ${canvasHeight}px.\n\nUser request:\n${userPrompt.trim()}`;
  if (!trendContext) return base;
  return `${base}

---
Current visual trend context (researched from live web, for inspiration only — do not copy literally, adapt where it fits the user's request):
${trendContext.trim()}
---`;
}
