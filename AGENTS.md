...
**현재 authority / design lock**: [`tooldi-agent-workflow-v6-layout-freedom-ssot.md`](./tooldi-agent-workflow-v6-layout-freedom-ssot.md) (2026-04-21 제정).

v6 SSOT 는 2026-04-20자 `Constrained HTML Pipeline SSOT (v5)` 를 **대체**한다. 제약 HTML grammar(`position:absolute` 강제, child 수 제한, line-break sibling 분해)와 deterministic DOM→Tooldi transpiler 경로는 폐기되었고, Phase 0 PoC 가 free HTML + 브라우저 렌더 + 결정적 primitive 추출로 같은 편집성을 더 적은 제약으로 달성 가능함을 증명했다.

## 현재 normative 문서

- [`tooldi-agent-workflow-v6-layout-freedom-ssot.md`](./tooldi-agent-workflow-v6-layout-freedom-ssot.md) — **철학/파이프라인/재사용·폐기 경계/모델 선택 SSOT**
- [`tooldi-natural-language-agent-v1-architecture.md`](./tooldi-natural-language-agent-v1-architecture.md) — runtime semantic contract (artifact identity, completion moment, Canvas Mutation Protocol 구조). v6와 정합.
- [`tooldi-agent-workflow-v1-functional-spec-to-be.md`](./tooldi-agent-workflow-v1-functional-spec-to-be.md) — public/API/persistence projection. v6 SSOT와 재정렬 중.
...
- [`tooldi-agent-workflow-v1-create-template-current-state-as-is.md`](./tooldi-agent-workflow-v1-create-template-current-state-as-is.md) — AS-IS current truth. v6 전환 진행 중 상태.
- [`tooldi-agent-workflow-v1-doc-index.md`](./tooldi-agent-workflow-v1-doc-index.md) — 읽기 순서 인덱스. v6 반영 상태.

## Historical (대체됨, 배경 참조만)

- [`tooldi-agent-workflow-v5-constrained-html-pipeline-ssot.md`](./tooldi-agent-workflow-v5-constrained-html-pipeline-ssot.md) — v6 SSOT 로 대체. 제약 HTML + deterministic transpile 경로의 설계 배경 이해 시에만.
...
1. `tooldi-agent-workflow-v6-layout-freedom-ssot.md`
   - 설계 철학 (P1~P5 원칙), 3단계 파이프라인(free HTML → browser layout → primitive extract), 재사용·폐기·신설 경계, 모델 선택 regime의 유일 SSOT.
...
- PM/아키텍처 리뷰:
  - `doc-index` → `v6 SSOT` → `architecture` → `scope-operations-decisions` → `functional-spec`

- Backend/worker 구현:
  - `doc-index` → `v6 SSOT` → `current-state-as-is` → `architecture` → `backend-boundary` → `functional-spec`

- FE/toolditor 구현:
  - `doc-index` → `v6 SSOT` → `current-state-as-is` → `architecture` → `client-boundary` → `functional-spec`

- 운영/QA/리뷰:
  - `doc-index` → `v6 SSOT` → `current-state-as-is` → `architecture` → `scope-operations-decisions` → `functional-spec`

## 수정 규칙

- 설계 철학(v6 원칙), 3단계 파이프라인, 재사용·폐기·신설 경계, 모델 선택 regime이 바뀌면:
  - 먼저 `v6 SSOT` 를 수정하고
...
- default 모델 교체(`gemini-3.1-flash-lite-preview` → 다른 모델):
  ...
  - `v6 SSOT` 의 §0.3 "증거 기반" 및 §6.2 "cost 프로파일"을 갱신하고
...
- 설계 철학 충돌 여부는 먼저 `v6 SSOT` 의 §1.2 원칙 표와 §1.3 anti-pattern 표를 본다.
...
- v6 전환이 코드 레벨로도 반영됐는지 확인하려면:
  ...
  rg -n "v5HtmlValidator|v5MethodBHtmlGen|v5MethodBSystemPrompt|v5PipelineOrchestrator|emitV5SkeletonMutations|v5Transpile|v5PipelineNode|V5PipelineDependencies|V5_APPLY_OPERATION" \
    agent-workflow-test/tooldi-agent-runtime --glob '!dist/**'
  ```
  0 hits가 전환 완료 조건.
