...
//
// Text bounds 는 element 의 content-box (padding + border inset) 로 계산한다.
...
function mapElement(el: V6RenderedElement): V6PrimitiveCommand[] {
...
function buildText(el: V6RenderedElement): V6PrimitiveCommand {
...
function buildImage(el: V6RenderedElement): V6PrimitiveCommand {
  const img = el.img;
  if (!img) {
    throw new Error(`buildImage called on element without img payload: ${el.path}`);
  }
  const opacity = parseOpacity(el.style.opacity);
  const transform = normalizeTransform(el.style.transform);
  const cmd = {
...
function buildSvg(el: V6RenderedElement): V6PrimitiveCommand {
  const svg = el.svg;
  if (!svg) {
    throw new Error(`buildSvg called on element without svg payload: ${el.path}`);
  }
  const opacity = parseOpacity(el.style.opacity);
  const transform = normalizeTransform(el.style.transform);
  const cmd = {
...
function hasVisiblePaint(el: V6RenderedElement): boolean {
  ...
  if (topBorderWidth && topBorderWidth > 0 && topBorderColor && topBorderColor.alpha > 0) {
    return true;
  }
  return false;
}
...
export function parseLinearGradient(bgImage: string): V6LinearGradient | null {
  ...
  const stops: V6GradientStop[] = stopParts.map((sp, idx) => {
    const colorMatch =
      /^(rgba?\([^)]+\)|#[0-9a-fA-F]+|[a-zA-Z]+)(?:\s+(\d+(?:\.\d+)?%))?$/.exec(sp);
...
function roundBounds(b: V6RenderedElement["bounds"]): V6RenderedElement["bounds"] {
  return {
    left: round(b.left),
    top: round(b.top),
    width: round(b.width),
