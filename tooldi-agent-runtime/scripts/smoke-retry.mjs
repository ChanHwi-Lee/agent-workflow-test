...
const objectStoreRootDir = await mkdtemp(
  join(tmpdir(), "tooldi-agent-runtime-retry-"),
);
...
try {
  try {
    ...
    console.log(
      "[retry-smoke] sandbox denied local TCP bind; using in-process retry fallback",
    );
    await runRetrySmokeInProcess({
      workspaceRoot,
      queueName,
    });
  }
    ...
    while (true) {
      ...
      while (delimiterIndex >= 0) {
        ...
        if (event.event === "run.log") {
          ...
          if (String(payload.message ?? "").includes("scheduled retry attempt 2")) {
            retryLogObserved = true;
            console.log("[retry-smoke] observed retry scheduling log");
          }
        }
        ...
        if (event.event === "run.failed") {
          throw new Error(`Run failed during retry smoke: ${event.data}`);
        }
        ...
        if (event.event === "run.completed") {
          ...
          console.log("[retry-smoke] observed run.completed SSE");
          return;
        }
      }
    }
